##path
import os
OPENSLIDE_PATH = r'C:/Users/anany/Desktop/gleason-grading/openslide-win64-20221217/bin'  #edit to correct openslide path
...
# ****** Opens image using Openslide in quarters, converts to grayscale, concatenate into final np array

slide = openslide.open_slide(str("/Users/anany/Desktop/gleason-grading/goodimg/"+ args.filename))
width, height = slide.dimensions
#arr = slide.read_region((0,0),0,(width,height))
...
halfheight = int(np.floor(height/2))
halfwidth = int(np.floor(width/2))
print(halfwidth, halfheight)

#top half
#gray1 = np.array(slide.read_region((0,0),0,(halfwidth,halfheight)).convert('L'))
#print('done with gray 1', gray1.shape)
#gray2 = np.array(slide.read_region((halfwidth,0),0,(halfwidth,halfheight)).convert('L'))
#print('done with gray 2', gray2.shape)

#bottom half
#gray3 = np.array(slide.read_region((0,halfheight),0,(halfwidth, halfheight)).convert('L'))
#print('done with gray 3', gray3.shape)
#gray4 = np.array(slide.read_region((halfwidth,halfheight),0,(halfwidth,halfheight)).convert('L'))
#print('done with gray 4', gray4.shape)

#concatenate
#gray5 = np.stack((gray1,gray2), axis=1)
#gray6 = np.stack((gray3,gray4), axis=1)
#gray = np.stack((gray5,gray6), axis=0)

#load in whole image
gray = np.array(slide.read_region((0,0),0,(width,height)).convert('L'))

print('made grayscale img')
...
#Creates annotation masks for each annotation image
annotation_mask = []
order_annotation = []
for i in annotation_image:
    print("annotation image", i)
    img_annotate = cv2.imread(i,0)
    #img_annotate = openslide.open_slide(i)
    #img_ann_arr = np.array(img_annotate.read_region((0, 0), 0, (row, column)))
    #img_annotate = cv2.imread(img_ann_arr,0)
    #img_ann_arr=[]
...
print('done with annotation loading')

# creates a dictionary of the number of pixels in each 256x256 patch in each category
def align_annotate(single_patch_index,annotation_mask, order_annotation):
    ...
    for i in range(len(annotation_mask)):
        ...
        patch_annotation = img_annotate[int(single_patch_index[0]):int(single_patch_index[1]),
                                        int(single_patch_index[2]):int(single_patch_index[3])]
...
print('saved to:', "/Users/anany/Desktop/gleason-grading/patch/"+args.output_dir)